...
void readDHT22 (DHT_DataTypedef *DHT_Data)
{
	...
	TIM3->CCR1 = 200;
	TIM3->CCR2 = 0;
	HAL_Delay(3000); //need to figure out delay
...
{
	readDHT22 (&DHT_Data);
	Temperature1 = DHT_Data.tCelsius;
	Humidity1 = DHT_Data.RH;
	*********************PWM Fans***********************
	 //if temperature gets to 17C and humidity 50%, shut off fans
	if (Temperature1 <= 22 && Humidity1 <= 50)
	{
		TIM2->CCR2 = 10;
		HAL_Delay(1);
	}
		 //if temperature gets to 23C and humidity 75%, run fans at 50%
	else if (Temperature1 >= 23 && Humidity1 <= 75)
	{
		TIM2->CCR2 = 500;
		HAL_Delay(1);
	}
